...
client.on("message", message => {
    if(message.content.toLowerCase() == "g!tag") 
    return message.channel.send(`☆`)
});
...
client.on("message", message => {
    if(message.content.toLowerCase() == "wixed") 
    return message.channel.send(`Wixed adamdır :heart: :heart:`)
});
...
let rol = member.guild.roles.cache.find(r => r.name === "SUSTURULDU");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('791571082511122483');
  }, ms(sürejail));
}
})
